...
import { staggerContainer, scaleIn } from '../components/PageTransition';
...
export default function MyQuizzesPage() {
  ...
  const [stats, setStats] = useState({ total_sessions: 0, total_students: 0 });
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-6 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <span className="text-2xl font-bold text-[var(--theme-text-main)]">{stats.total_sessions}</span>
          <span className="text-sm text-[var(--theme-text-dim)]">Sessions Run</span>
        </div>
        <div className="w-px h-8 bg-[var(--border-default)]" />
        <div className="flex items-center gap-2">
          <span className="text-2xl font-bold text-primary">{stats.total_students}</span>
